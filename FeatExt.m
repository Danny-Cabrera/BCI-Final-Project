...
for i= 1:NWindows
    FreqFeats(1,i) = mean(mean(abs(S(5:15,i:i+1)))); %the rows correspond with selected freq bands
    FreqFeats(2,i) = mean(mean(abs(S(20:25,i:i+1))));
    FreqFeats(3,i) = mean(mean(abs(S(75:115,i:i+1)))); % note paper uses diff freq bins
    FreqFeats(4,i) = mean(mean(abs(S(125:160,i:i+1))));
    FreqFeats(5,i) = mean(mean(abs(S(160:175,i:i+1))));
end
...
for i=1:NWindows
    TimeFeat(1,i)=mean(x(NOverlap*(i-1)+1:NOverlap*(i-1)+window));
end
